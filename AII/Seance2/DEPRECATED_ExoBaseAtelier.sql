-- !!! A REMPLIR !!!
-- Nom prenom numero etu email

-- Exercice Base De Donnée "ptiteusine"

-- Créer une nouvelle base de données "ptiteusine"

-- La ptiteusine contient des machines
-- Les machines ont un type: Tour/Fraiseuse/Robot
-- OKUMA1 (tour), OKUMA2 (tour), RobotFanuc, RobotUR, Biglia(fraiseuse), Index (fraiseuse).


-- La ptiteusine contient des employes


-- Les employes ont un métier
-- Soit maintenance, operateur, regleur
-- 2 à la maintenance, 5 opérateurs, 2 regleurs

-- Les operateurs sont formés à travailler sur certaines machines


-- Pensez à intégrer les contraintes utiles dans le schéma de la base de données

-- Inspirez vous du script seance2/ptiteusineBDD.sql
-- SVP gardez la premiere ligne \c postgres pour me faciliter la vie.. merci ! :)

-- Envoyer votre travail à coutaudu@gmail.com

-- Le "compte-rendu" sera directement le fichier SQL (renommer en nom_ptiteusine.sql)
-- Remarque pas d'espace dans le nom du fichier !!!!!!!

-- Mettez les commentaires nécessaires à la compréhension.


-- Preparer un script de requêtes SQL pour afficher:
-- Le prenom des employes avec les machines auxquelles ils sont formés.
-- La liste des employes qui sont formé à travailler sur un robot.


